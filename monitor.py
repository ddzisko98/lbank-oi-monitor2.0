...
# Регексп шукає "Позиції", потім будь-які символи (перенос рядка, пробіли,
# тире), потім число, потім "(CXMT)". Число може містити крапку/кому.
...
def fetch_open_interest() -> float:
    with sync_playwright() as p:
        ...
        value = None
        # Значення підвантажується асинхронно, тому пробуємо кілька разів,
        # поки не з'явиться реальне число замість "--".
        for _ in range(15):
